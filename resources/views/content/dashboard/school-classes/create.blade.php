...
@section('title', __('Create School Class'))
...
<div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="py-3 mb-4">
        <span class="text-muted fw-light">{{ __('School Classes') }} /</span> {{ __('Create New Class') }}
    </h4>

    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0">{{ __('Class Details') }}</h5>
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route('dashboard.school-classes.store') }}">
                @csrf
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12 mt-4">
                        <button type="submit" class="btn btn-primary me-sm-3 me-1">{{ __('Create Class') }}</button>
                        <a href="{{ route('dashboard.school-classes.index') }}" class="btn btn-label-secondary">{{ __('Cancel') }}</a>
                    </div>
                </div>
            </form>
        </div>
