...
@section('content')
<h4 class="py-3 mb-4">
  <span class="text-muted fw-light">{{ __('Semesters') }} /</span> {{ __('Create') }}
</h4>
  ...
  <div class="col-md-12">
    <div class="card mb-4">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="mb-0">{{ __('Add New Semester') }}</h5>
      ...
      <div class="card-body">
        <form action="{{ route('dashboard.semesters.store', ['country' => $country]) }}" method="POST">
          @csrf
          ...
          <div class="mb-3">
            ...
            <select class="form-select @error('grade_level') is-invalid @enderror" 
              ...
              @foreach($classes as $class)
                <option value="{{ $class->id }}" {{ old('grade_level') == $class->id ? 'selected' : '' }}>
                  {{ $class->grade_name }}
                </option>
              @endforeach
            </select>
